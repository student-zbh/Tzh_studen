...
func (con ManagerController) DoAdd(c *gin.Context) {
	...
	if len(username) < 2 || len(password) < 6 {
		con.Error(c, "用户名或密码不合法", "/admin/manager/add")
		return
	}
...
func (con ManagerController) DoEdit(c *gin.Context) {
	...
	role_id, err2 := models.Int(c.PostForm("role_id"))
	if err2 != nil {
		con.Error(c, "传入数据错误", "/admin/manager")
	}
	// 获取表单传入数据
	...
	// 判断密码是否为空 为空表示不修改，不为空表示修改
	if password != "" {
		if len(password) == 6 {
			manager.Password = models.Md5(password)
		} else {
			con.Error(c, "密码长度错误", "/admin/manager/edit?id="+models.String(id))
		}
	}
